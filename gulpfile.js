...
const appendJsExtension = () => new Transform({
  objectMode: true,
  transform(file, enc, cb) {
    if (file.isBuffer()) {
      let code = file.contents.toString(enc)
      code = code.replace(/((?:import|export)[^'"]*['"]\.[^'"]*)(['"])/g, (match, start, quote) => {
        if (/\.[a-z0-9]+$/i.test(start)) return match
        return `${start}.js${quote}`
      })
      file.contents = Buffer.from(code, enc)
    }
    cb(null, file)
  }
})
